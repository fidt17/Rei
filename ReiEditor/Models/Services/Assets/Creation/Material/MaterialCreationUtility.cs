...
public class MaterialCreationUtility : IMaterialCreationUtility
{
    ...
    public async Task<bool> CreateMaterialAsync(MaterialCreationSettings settings)
    {
        try
        {
            ...
            if (!_shaderRegistry.TryGetById(settings.ShaderAssetId, out var shaderAsset)) throw new Exception($"Shader asset '{settings.ShaderAssetId}' does not exist");
            ...
            var material = new global::ReiEditor.Models.Services.Render.Material(shaderAsset.AssetId);
