...
class hello_triangle_e3 final : public rei::render::BaseRenderScenario
{
public:
    explicit hello_triangle_e3(GLFWwindow* target)
    ...
    void Setup() override
    {
        _firstTriangleShader = rei::render::ShaderUtility().CreateShaderProgram(vertexShaderSource, redShaderSource);
        _secondTriangleShader = rei::render::ShaderUtility().CreateShaderProgram(vertexShaderSource, blueShaderSource);
    ...
    void Render() override
    {
        ...
        if (counter++ % 100 < 50)
        {
            glUseProgram(_firstTriangleShader);
            glBindVertexArray(_firstTriangle);
            glDrawArrays(GL_TRIANGLES, 0, 6);
        }
        else
        {
            glUseProgram(_secondTriangleShader);
...
private:
    unsigned int _firstTriangleShader;
    unsigned int _secondTriangleShader;
