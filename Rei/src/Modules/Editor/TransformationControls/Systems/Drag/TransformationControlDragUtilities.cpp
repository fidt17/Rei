...
namespace rei::editor::transformation_control_drag
{
    ...
    bool ShouldStartPointerDrag(const physics::PointerCollisionListener& pointerListener)
    {
        return pointerListener.IsInside || (Input::IsMouseButtonPressed(GLFW_MOUSE_BUTTON_LEFT) && pointerListener.DidExit);
    }
    ...
    void CaptureDragStartTargetStates(const std::shared_ptr<ecs::World>& world, TransformationControl& control)
    {
        ...
        for (const auto entity : control.TargetEntities)
        {
            ...
            {
                const auto& rectTransform = GET(entity, ui::RectTransform);
                targetState.AnchoredPosition = rectTransform.GetAnchoredPosition();
                targetState.SizeDelta = rectTransform.GetSizeDelta();
            }
