...
public class HierarchyService : IHierarchyService, IDisposable
{
    ...
    public HierarchyService(ISceneManagementService sceneManagementService, IEntityManagementService entityManagementService)
    {
        ...
        _entityManagementService.EntityMovedEvent += HandleEntityMovedEvent;
    ...
    private void HandleEntityCreatedEvent(GameEntity e) => _activeHierarchy.Value?.AddNode(new Hierarchy.Node(e, null));
    private void HandleEntityDeletedEvent(GameEntity e) => _activeHierarchy.Value?.RemoveNodeWhere(n => n.Entity == e);
    
    private void HandleEntityMovedEvent(GameEntity e)
    {
        var scene = _sceneManagementService.CurrentScene.Value;
        if (scene == null) return;
        _activeHierarchy.Value = new Hierarchy(scene);
    }
}