...
Active scene regressions check duplicate hierarchy insertion atomicity, cross-parent move source index, oversized same-parent insertion, and preservation of a surviving child when its old parent disappears from an engine snapshot. Their production fixes remain separate work. These failures remain in the default run and are not skipped or filtered out.
